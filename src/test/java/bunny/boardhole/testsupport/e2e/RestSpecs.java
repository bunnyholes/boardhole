...
final class RestSpecs {
    ...
    public static RequestSpecification form() {
        return given().contentType(ContentType.URLENC).accept(ContentType.JSON);
    }
    ...
    public static RequestSpecification formAuth(SessionCookie session) {
        return given().contentType(ContentType.URLENC).accept(ContentType.JSON).cookie(session.name(), session.value());
    }
}
