...
public final class AuthSteps {
    ...
    public static void signup(String username, String password, String name, String email) {
        RestSpecs.anon().contentType(ContentType.URLENC).formParams(Map.of("username", username, "password", password, "name", name, "email", email)).when().post("auth/signup").then().statusCode(anyOf(is(204), is(409)));
    }

    public static SessionCookie login(String username, String password) {
        var res = RestSpecs.anon().contentType(ContentType.URLENC).formParams(Map.of("username", username, "password", password)).when().post("auth/login").then().statusCode(204).extract().response();
