...
public final class ProblemDetailsMatchers {
    ...
    public static ResultMatcher[] unauthorized() {
        return new ResultMatcher[] {
            jsonPath("$.type").value("urn:problem-type:unauthorized"),
            jsonPath("$.title").value(MessageUtils.get("exception.title.unauthorized")),
            jsonPath("$.status").value(401),
            jsonPath("$.code").value(ErrorCode.UNAUTHORIZED.getCode())
        };
    }

    public static ResultMatcher[] forbidden() {
        return new ResultMatcher[] {
            jsonPath("$.type").value("urn:problem-type:forbidden"),
            jsonPath("$.title").value(MessageUtils.get("exception.title.access-denied")),
            jsonPath("$.status").value(403),
            jsonPath("$.code").value(ErrorCode.FORBIDDEN.getCode())
        };
    }

    public static ResultMatcher[] notFound() {
        return new ResultMatcher[] {
            jsonPath("$.type").value("urn:problem-type:not-found"),
            jsonPath("$.title").value(MessageUtils.get("exception.title.not-found")),
    ...
    public static ResultMatcher[] validationError() {
        return new ResultMatcher[] {
            jsonPath("$.type").value("urn:problem-type:validation-error"),
            jsonPath("$.title").value(MessageUtils.get("exception.title.validation-failed")),
